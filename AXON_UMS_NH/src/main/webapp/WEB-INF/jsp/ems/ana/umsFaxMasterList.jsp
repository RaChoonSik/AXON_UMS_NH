...
<div class="graybox">
	...
	<div class="grid-area">
		<table class="grid">
			...
			<tbody>
				<c:if test="${fn:length(faxMasterList) > 0}">
					<!-- 데이터가 있을 경우// -->
					<c:forEach items="${faxMasterList}" var="faxSend" varStatus="faxSendStatus">
						<tr>
							<td><c:out value="${pageUtil.totalRow - (pageUtil.currPage-1)*pageUtil.pageRow - faxSendStatus.index}" /></td>  
							<td><a href="javascript:;" onclick="goFaxMasterDetil(this);" class="bold"><c:out value="${faxSend.umsFaxMasterSeq}" /></a></td>
							...
							<td><c:out value="${faxSend.targetCnt}" /></td>
							<td><c:out value="${faxSend.successCnt}" /></td>
							<td><c:out value="${faxSend.failCnt}" /></td>
							<td><c:out value="${faxSend.waitCnt}" /></td>
							...
							<td class="hide"><c:out value="${faxSend.attach01}" /></td>
							<td class="hide"><c:out value="${faxSend.attach02}" /></td>
							<td class="hide"><c:out value="${faxSend.attach03}" /></td>
						</tr>
					</c:forEach>
					<!-- //데이터가 있을 경우 -->
				</c:if>

				<c:if test="${empty faxMasterList}">
					<!-- 데이터가 없을 경우// -->
					<tr>
