...
function checkForm() {
	...
	if(getByteStr($("#itDevNm").val()) > 20 ) {
		alert("IT개발자는 20byte를 넘을 수 없습니다.");
		$("#itDevNm").focus();
		$("#itDevNm").select();
		errflag = true;
	}
...
/*
function goCancelPop2() {
	alert(1111);
	parent.fn.popupClose('#popup_ifBasicListP');
	//$("#ifFormVo").attr("target","").attr("action","./ifBasicListP.ums").submit();
	parent.goIfBasciListPop();
}
*/
		...
		$.ajax({
			...
			success: function (result) {
				if( result.successYn == 'Y'){
					alert("파일 업로드가 완료되었습니다. 반영위해 등록버튼 클릭해주세요.");
					fn.popupClose('#popup_file_mapp');
					getifBasicAddList();
				}else{
