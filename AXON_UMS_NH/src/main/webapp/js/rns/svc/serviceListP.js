...
function goSearch(pageNo) {
	...
	$.ajax({
		type : "GET",
		url : "/rns/svc/serviceList.ums?" + param,
		dataType : "html",
...
function goServiceAdd() {
	document.location.href = "/rns/svc/serviceAddP.ums";
}
...
function goApprCancel() {
	...
	$("#status").val("000");
	var param = $("#searchForm").serialize();
	$.getJSON("/rns/svc/apprCancel.json?" + param, function(data) {
...
function goDisable() {
	...
	$("#status").val("001");
	var param = $("#searchForm").serialize();
	$.getJSON("/rns/svc/updateServiceStatus.json?" + param, function(data) {
		if(data.result == "Success") {
			alert("사용중지되었습니다.");
...
function goUpdate(tid) {
	$("#tid").val( tid );
	$("#searchForm").attr("target","").attr("action","/rns/svc/serviceUpdateP.ums").submit();
}
...
/*function goUpdateDate(tid, approvalProcAppYn) {
	...
	$("#searchForm").attr("target","").attr("action","/rns/svc/serviceUpdateDateP.ums").submit();
}*/
...
function popSubmitApproval(tid, userId, prohibitChkTyp) {
	...
	if (prohibitChkTyp == "002"){
		if(confirm("준법심의 대상정보가 포함되어있어 준법감시부 승인 후 발송해야합니다. 계속 진행하시겠습니까?")){
			...
			$.ajax({
				type : "GET",
				url : "/rns/svc/pop/popRnsApprList.ums?" + param,
				dataType : "html",
				success : function(pageHtml){
...
function goRnsSubmitApproval(tid) {
	if(confirm("보안결재를 상신 합니다.")) {
		...
		$.getJSON("/rns/svc/rnsSubmitApproval.json?" + param, function(data) {
			if(data.result == "Success") {
				alert("상신 되었습니다.");
...
function popRnsApprovalState(tid) {
	...
	$.ajax({
		type : "GET",
		url : "/rns/svc/pop/popRnsApprStateList.ums?" + param,
		dataType : "html",
		success : function(pageHtml){
...
/*	if (a) {
		var param = $("#searchForm").serialize();
		$.getJSON("/rns/svc/mailAdmit.json?" + param, function(data) {
			if (data.result == "Success") {
				alert("발송승인 되었습니다.");
...
function checkTid(){
    ...
    $.getJSON("/rns/svc/tidCheck.json?tid=" + copyTid , function(data) {
        if(data.result == "Success") { 
            $("#popChkTid").val("Y");
...
function goPopCopy(){
    var param = $("#searchForm").serialize();
    $.getJSON("/rns/svc/serviceCopy.json?" + param, function(data) {
        if(data.result == "Success") {
