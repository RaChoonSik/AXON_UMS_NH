...
function setServiceContent(contentsTyp, contentsPath) {
	...
	$.getJSON("/rns/svc/serviceFileView.json?contentsPath=" + contentsPath, function(res) {
...
function goDelete() {
	...
	$.getJSON("/rns/svc/updateServiceStatus.json?" + param, function(data) {
		if(data.result == "Success") {
			...
			$("#searchForm").attr("target","").attr("action","/rns/svc/serviceListP.ums").submit();
		} else if(data.result == "Fail") {
...
/*	if($("#approvalYn").val() == "N") {
	...
	$("#serviceInfoForm").attr("target","iFrmService").attr("action","/rns/svc/serviceUpdate.ums").submit();
}
	...
	if(rnsStatus == "000" && rnsWorkStatus == "201") {
		...
		*/
		...
		if(confirm("보안결재를 상신 합니다.")) {
			...
			$.ajax({
				type: "POST",
				url: '/rns/svc/rnsSubmitApprovalP.json',
				data: frmData,
				processData: false,
				contentType: false,
