 ...
 * 작성자 : 김준희
 * 작성일시 : 2024.03.05
 * 설명 : RNS API Controller
 */
package kr.co.enders.ums.api.rns.controller;
	...
	public void goApiSendTsEmailList(@RequestBody Map<String, Object> params, Model model, HttpServletRequest request, HttpServletResponse response, HttpSession session) {
		...
		if ("000".equals(sResultcode)) {
			try {
				...
				searchVO.setTid(templatecode);
				searchVO.setUilang("000");
				RnsServiceVO templateVO  = rnsServiceService.getServiceInfo(searchVO);
				if (templateVO == null) {
		...
		if (!"000".equals(sResultcode)) {
			...
			//DB 작업 시작 
			RnsRecipientInfoVO rnsRecipientInfoVO = new RnsRecipientInfoVO(); 
			rnsRecipientInfoVO.setSubid(subid);
			rnsRecipientInfoVO.setTid(templatecode);
			rnsRecipientInfoVO.setSpos(spos);
			rnsRecipientInfoVO.setSid(senderid);
			rnsRecipientInfoVO.setSname(sendername);
			rnsRecipientInfoVO.setSmail(senderemail);
			rnsRecipientInfoVO.setRpos(rpos);
			rnsRecipientInfoVO.setCtnpos(ctnpos);
			rnsRecipientInfoVO.setSubject(messagename);
			rnsRecipientInfoVO.setContents(message);
			rnsRecipientInfoVO.setRequestKey(requestkey);
			rnsRecipientInfoVO.setRid(memberno);
			rnsRecipientInfoVO.setRname(membername);
			rnsRecipientInfoVO.setRmail(receiveemail);
			rnsRecipientInfoVO.setMap1(map1);
			rnsRecipientInfoVO.setMap2(map2);
			rnsRecipientInfoVO.setMap3(map3);
			rnsRecipientInfoVO.setMap4(map4);
			rnsRecipientInfoVO.setMap5(map5);
			rnsRecipientInfoVO.setMap6(map6);
			rnsRecipientInfoVO.setMap7(map7);
			rnsRecipientInfoVO.setMap8(map8);
			rnsRecipientInfoVO.setMap9(map9);
			rnsRecipientInfoVO.setMap10(map10);
			rnsRecipientInfoVO.setMap11(map11);
			rnsRecipientInfoVO.setMap12(map12);
			rnsRecipientInfoVO.setMap13(map13);
			rnsRecipientInfoVO.setMap14(map14);
			rnsRecipientInfoVO.setMap15(map15);
			rnsRecipientInfoVO.setAttachfile01(attachfile01);
			rnsRecipientInfoVO.setAttachfile02(attachfile02);
			rnsRecipientInfoVO.setAttachfile03(attachfile03);
			rnsRecipientInfoVO.setAttachfile04(attachfile04);
			rnsRecipientInfoVO.setAttachfile05(attachfile05); 
			
			rnsRecipientInfoVO.setRequestKey(requestkey);
			rnsRecipientInfoVO.setStatus(status);
			  
			try {
				int result = rnsServiceService.insertApiSendTsEmail(rnsRecipientInfoVO);
				if (result < 1 ) {
					sResultcode = "E004";
