package cust.forbidden.forbiddata;
...
public class ForbiddenData {
	...
	public synchronized  String getProhibitWordApi(String title, String content) {
		...
		String networkEnv = "PRD";
		
		if(!StringUtil.isNull(properties.getProperty("NETWORK.ENV"))) {
			networkEnv = properties.getProperty("NETWORK.ENV");
		} 
		
		if ("DEV".equals(networkEnv)) {
			return getProhibitWordApiTest(title, content );
		}
		
		apiUrl = properties.getProperty("PROHIBIT.CHECK_URL"); //API URL 입력 
